...
p(d | q) = exp(f(d, q)) / Σd′ ∈ D exp(f(d′, q))
...
Training can be motivated as maximizing likelihood of the document corresponding to each query, i.e. Σj log p(dj | qj). Unfortunately, since retrieval datasets can have D exceed millions of documents, computing the normalizer in Eq 1 at each training step is not an option. Instead contrastive learning is used where the likelihood is replaced with a biased approximation calculated from negative samples:

max log p(dj qj) ≈ log exp f(dj, qj)

ϕ,ψj d′ ∈ H(qj) exp f(d′, qj)
...
max log p(d, q) = max log exp f(d, q)
...
(B1, Bmax B) f(d, q′) + f(d, q) = (B1, BB)′ max ϕ(d) · ψ(q′) + ϕ(d) · ψ(q)
...
log p(d q) = exp f (d, q) + exp f (d, q)d ′ S(q,d) exp f (d′ , q)
...
Here M1 is the first-stage encoder model, M2 is a second-stage encoder model, and E is the token embedding matrix of M2 applied to each token in d′. In practice, we parameterize both M1 and M2 using traditional bidirectional transformers, so our model is comprised of two biencoder-like backbones called in sequence.
...
We observe that larger batches bring easier non-negative examples, and decreasing cluster size clearly increases the average hardness of negative examples in a given cluster.

# Figure 4:

across various batch and cluster sizes. Each dot is a biencoder pretrained with a different batch and cluster size.

# Impact of filtering during training

# Figure 5: Performance on MTEB across epochs

of supervised training on the Nomic and BGE supervised meta-datasets.

|Epochs of supervised training|Nomic (1 mined negative)|BGE (1 mined negative)|BGE (0 mined negatives)|
|---|---|---|---|
|0.58|256|39|Android|
|0.56|Mathematica|Physics|Programmers|
|0.54|Stats|Tex|Unix|
|0.52|Webmasters|Wordpress| |
|0.50| | | |
|0.48| | | |
|0.46| | | |
...
as measured by a surrogate model as cluster size and batch size change.

# Figure 7: Impact of context by testing our model with different Stackexchange forum input types.

Y-axis indicates the input domain, X-axis indicates the test domain. Dark squares come within one point NDCG@10.
...
Hongjin Su, Weijia Shi, Jungo Kasai, Yizhong Wang, Yushi Hu, Mari Ostendorf, Wen tau Yih, Noah A. Smith, Luke Zettlemoyer, and Tao Yu. One embedder, any task: Instruction-tuned text embeddings, 2023.
...
|ANCE|docTSquery|CoIBERT|docTSquery|
|---|---|---|---|
|0.15|hotpotqa|cifactever|ebis-touche2020|
|0.15|webis-tARE020|corpus fever|0-0Z|
|0.10|climate|0.10|limate-fever|
|0.05|quora|cidocs|gudna|
|0.00|scidocs|fever chmeotpus|climate-fever|
|0.00|rec|arguana|rec-covidever|

Figure 8: Analysis of domain shift for popular neural retrieval methods. Performance difference from BM25 (y-axis) correlates with the different in IDF of the test corpus D from the training corpus dqatqa D T.
...
# 10.3 INTERACTIONS BETWEEN CONTRASTIVE LOSS AND DISTRIBUTED DATA

The authors note that it can be notoriously difficult to train models using both contrastive loss and the distributed data parallel (DDP) setting. In particular, when aggregating samples between GPUs, if any artifact reveals which GPU a model came from (for example, if the GPU model weights are initialized slightly differently) than the model can quickly deteriorate to a suboptimal solution.

|IDF Distance from MSMARCO|Distance from MSMARCO|
|---|---|
|0.15|quora|
|0.15|limate-fever|
|0.10|ever|
...
|Batch size: 64|Batch size: 256|Batch size: 512|Batch size: 1024|Batch size: 2048|Batch size: 4096|Batch size: 8192|Batch size: 16384|Batch size: 32768|Batch size: 65536|Batch size: 131072|Batch size: 262144|Batch size: 524288|Batch size: 1048576|Batch size: 2097152|
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
|47.8|48.4|49.0|48.9|49.2|49.1|49.0|49.3|48.6|48.4|48.2|48.9|49.15|49.0|49.0|
|Corr: 0.64|Corr: 0.71|Corr: 0.29|Corr: 0.86|Corr: 0.93|Corr: 0.50| | | | | | | | | |
...
Section 10.5. Our experiments in this setting (using the mined negatives from the Nomic supervised meta-datasets) generally show decreasing performance with additional hard negatives.
...
Breckenridge, Michigan. Breckenridge is a village in Gratiot County in the U.S. state of Michigan. The population was 1,328 at the 2010 census. The village is located in Wheeler Township.
...
Manson, Iowa. Manson is a city in Calhoun County, Iowa, United States. The population was 1,690 at the 2010 census.

# What happens after a sentencing hearing?

After a criminal defendant is convicted or pleads guilty, a judge will decide [...]

# Athead County Population

Athead County, Montana. Athead County is a county located in the U.S. state of Montana. As of the 2010 census, the population was 90,928, making it [...]
...
Lewiston, ID population and races. As of 2010-2014, the total population of Lewiston is 32,178, which is 4.12% more than it was in 2000. [...]
...
|Yahoo Title-Answerc|262,144|&lt;.01|
...
|StackExchange Duplicate Questionsf|65,536|&lt;.01|
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
|Unsupervised Baseline|15.7|54.8|41.4|24.7|40.2|74.4|39.9|63.8|35.0|35.7|48.6|88.2|20.2|72.0|62.2|19.2|48.0|
